...
3. **1행**에 아래 열 제목을 **왼쪽부터 순서대로** 붙여 넣습니다. (탐색·정렬은 같은 배열로 순차+이진, 버블+삽입+퀵을 모두 제출하는 형식입니다.)
...
| E | 탐색배열JSON | 정렬된 배열 `[...]` |
| F | 목표값 | 숫자 |
| G | 선형경로답 | 순차 탐색 경로 — JSON 배열 문자열 (열 이름은 구현 호환용으로 `선형` 접두사 유지) |
| H | 선형횟수답 | 순차 탐색 비교 횟수 — 문자열 |
| I | 선형정답 | 순차 탐색 채점 — 경로·횟수 각각 `O`/`X` 두 글자 (`OO` `OX` `XO` `XX`, 앞=경로 뒤=횟수) |
| J | 이진경로답 | JSON 배열 문자열 |
| K | 이진횟수답 | 문자열 |
| L | 이진정답 | 순차 탐색과 동일 형식 (`OO`~`XX`, 앞=경로 뒤=횟수) |
| M | 정렬초기배열JSON | 원본(섞인) 배열 |
| N | 버블1회답 | JSON 배열 문자열 |
| O | 버블2회답 | JSON 배열 문자열 |
| P | 버블정답 | 1·2회전 채점 2글자: `OO` `OX` `XO` `XX` (앞=1회전, 뒤=2회전) |
| Q | 삽입1회답 | JSON 배열 문자열 |
| R | 삽입2회답 | JSON 배열 문자열 |
| S | 삽입정답 | 위와 동일 (`OO`~`XX`) |
| T | 퀵1회답 | JSON 배열 문자열 |
...
| V | 퀵정답 | 위와 동일 (`OO`~`XX`) |
| W | 효율1상황 | 문장 |
| X | 효율1답 | 학생 입력(공백 제거 후 저장) `순차탐색`/`이진탐색` (`선형탐색`도 채점에서 인정) |
| Y | 효율1정답 | 채점 `O`/`X` |
| Z | 효율2상황 | 문장 |
| AA | 효율2답 | 학생 입력 |
| AB | 효율2정답 | 채점 `O`/`X` |
