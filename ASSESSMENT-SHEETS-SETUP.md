...
| G | 선형경로답 | JSON 배열 문자열 |
| H | 선형횟수답 | 문자열 |
| I | 선형정답 | 경로·횟수 각각 `O`/`X` 두 글자 (`OO` `OX` `XO` `XX`, 앞=경로 뒤=횟수) |
| J | 이진경로답 | JSON 배열 문자열 |
| K | 이진횟수답 | 문자열 |
| L | 이진정답 | 선형과 동일 (`OO`~`XX`, 앞=경로 뒤=횟수) |
...
- **선형정답·이진정답**: 비교 **경로**와 **횟수**를 따로 채점해 두 글자 (`OO`~`XX`, 첫째=경로, 둘째=횟수). 횟수 칸이 비었거나 숫자가 아니면 둘째는 `X`.
